...
#ifdef __WIN__
static const char *f_extensions[]= { ".ini", ".conf", 0 };
#define NEWLINE "\r\n"
#else
static const char *f_extensions[]= { ".conf", 0 };
#define NEWLINE "\n"
#endif
...
#endif
  if (!(fp= mysql_file_fopen(key_file_conf, name, O_RDONLY, MYF(0))))
  ...
  while (mysql_file_fgets(buff, sizeof(buff) - 1, fp))
  {
    ...
    if (*ptr == '!')
    {
               ...
               my_isspace(&my_charset_latin1, ptr[sizeof(include_keyword)-1]))
      {
        ...
        int errnum = search_default_file_with_ext(opt_handler, handler_ctx, "", "", ptr,
                                     recursion_level + 1);
...
static const char **init_default_directories(MEM_ROOT *alloc)
{
  ...
  if (DEFAULT_SYSCONFDIR[0]) {
    ...
    if (path_ptr != NULL) {
      errors += add_directory(alloc, path_ptr, dirs);
    }
 }
#else
