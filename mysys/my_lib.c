...
MY_DIR	*my_dir(const char *path, myf MyFlags)
{
  ...
  DBUG_ENTER("my_dir");
  DBUG_PRINT("mariadb",("path: '%s' MyFlags: %lu",path,MyFlags));
...
#endif
  DBUG_ENTER("my_dir");
  DBUG_PRINT("mariadb",("path: '%s' MyFlags: %d",path,(int)MyFlags));
...
{
  DBUG_ENTER("my_fstat");
  DBUG_PRINT("mariadb",("fd: %d  MyFlags: %lu", Filedes, MyFlags));
#ifdef _WIN32
...
MY_STAT *my_stat(const char *path, MY_STAT *stat_area, myf my_flags)
{
  int m_used;
  DBUG_ENTER("my_stat");
  DBUG_PRINT("mariadb", ("path: '%s'  stat_area: %p  MyFlags: %lu", path,
                    stat_area, my_flags));
