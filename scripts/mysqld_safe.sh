...
find_in_bin() {
  if test -x "$MARIADB_BASEDIR_VERSION/bin/$1"
  then
    echo "$MARIADB_BASEDIR_VERSION/bin/$1"
...
parse_arguments() {
  for arg do
    ...
    case "$arg" in
      ...
      --basedir=*) MARIADB_BASEDIR_VERSION="$val" ;;
	...
	;;
      ...
      # these might have been set in a [mysqld_safe] section of mariadb.conf
      # they are added to mysqld command line to override settings from mariadb.conf
      --skip[-_]log[-_]error)
        err_log=;
        skip_err_log=1;
        ;;
      ...
      # mysqld_safe-specific options - must be set in mariadb.conf ([mysqld_safe])!
...
if test -n "$MARIADB_BASEDIR_VERSION" -a -d "$MARIADB_BASEDIR_VERSION"
then
  # BASEDIR is already overridden on command line.  Do not re-set.

  # Use BASEDIR to discover le.
  if test -x "$MARIADB_BASEDIR_VERSION/libexec/mysqld"
  then
    ledir="$MARIADB_BASEDIR_VERSION/libexec"
  elif test -x "$MARIADB_BASEDIR_VERSION/sbin/mysqld"
  then
    ledir="$MARIADB_BASEDIR_VERSION/sbin"
  else
    ledir="$MARIADB_BASEDIR_VERSION/bin"
  fi
elif test -x "$MY_PWD/bin/mysqld"
then
  MARIADB_BASEDIR_VERSION="$MY_PWD"		# Where bin, share and data are
  ledir="$MY_PWD/bin"			# Where mysqld is
# Check for the directories we would expect from a source install
elif test -x "$MY_PWD/libexec/mysqld"
then
  MARIADB_BASEDIR_VERSION="$MY_PWD"		# Where libexec, share and var are
  ledir="$MY_PWD/libexec"		# Where mysqld is
elif test -x "$MY_PWD/sbin/mysqld"
then
  MARIADB_BASEDIR_VERSION="$MY_PWD"		# Where sbin, share and var are
  ledir="$MY_PWD/sbin"			# Where mysqld is
# Since we didn't find anything, used the compiled-in defaults
else
  MARIADB_BASEDIR_VERSION='@prefix@'
  ledir='@libexecdir@'
fi
...
if test -d $MARIADB_BASEDIR_VERSION/data/mysql
then
  DATADIR=$MARIADB_BASEDIR_VERSION/data
# Next try where the source installs put it
elif test -d $MARIADB_BASEDIR_VERSION/var/mysql
then
  DATADIR=$MARIADB_BASEDIR_VERSION/var
# Or just give up and use our compiled-in default
else
  DATADIR=@localstatedir@
fi

if test -z "$MARIADB_HOME"
then 
  if test -r "$DATADIR/mariadb.conf"
  then
    log_error "WARNING: Found $DATADIR/mariadb.conf
The data directory is not a valid location for mariadb.conf, please move it to
$MARIADB_BASEDIR_VERSION/mariadb.conf"
  fi
  MARIADB_HOME=$MARIADB_BASEDIR_VERSION
fi
export MARIADB_HOME
...
# Get first arguments from the mariadb.conf file, groups [mysqld] and [server]
...
# If arguments come from [mysqld_safe] section of mariadb.conf
...
if [ -n "${PLUGIN_DIR}" ]; then
  ...
  do
    if [ -d "${MARIADB_BASEDIR_VERSION}/${dir}" ]; then
      plugin_dir="${MARIADB_BASEDIR_VERSION}/${dir}"
      break
    fi
  done
...
safe_mysql_unix_port=${mysql_unix_port:-${MYSQL_UNIX_PORT:-@MARIADB_UNIX_ADDR@}}
...
# and sort_buffer values to mariadb.conf to improve check performance or require
# less disk space.
# Alternatively, you can start mysqld with the "myisam-recover" option. See
# the manual for details.
#
# echo "Checking tables in $DATADIR"
# $MARIADB_BASEDIR_VERSION/bin/myisamchk --silent --force --fast --medium-check $DATADIR/*/*.MYI
# $MARIADB_BASEDIR_VERSION/bin/isamchk --silent --force $DATADIR/*/*.ISM
...
for i in  "$ledir/$MYSQLD" "$defaults_group_suffix" "$defaults" "--basedir=$MARIADB_BASEDIR_VERSION" \
