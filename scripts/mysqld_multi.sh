...
sub list_defaults_files
{
  ...
  if ('@sysconfdir@') {
    push @dirs, '@sysconfdir@/mariadb.conf';
  } else {
    push @dirs, '/etc/mariadb.conf', '/etc/mariadb/mariadb.conf';
  }
  push @dirs, "$ENV{MARIADB_HOME}/mariadb.conf" if $ENV{MYSQL_HOME};
  push @dirs, $opt{'extra-file'} if $opt{'extra-file'};
  push @dirs, "$ENV{HOME}/.mariadb.conf" if $ENV{HOME};
...
{
  print <<EOF;
# This is an example of a mariadb.conf file for $my_progname.
# Usually this file is located in home dir ~/.mariadb.conf or @sysconfdir@/mariadb.conf
#
...
#
#   Before using, you must create a mariadb.conf file either in @prefix@/mariadb.conf
#   or /root/.mariadb.conf and add the [mysqld_multi] and [mysqld#] groups.
#
...
$my_progname can read group [mysqld_multi] from mariadb.conf file. You may
...
$my_progname will search for groups named [mysqld#] from mariadb.conf (or
...
--mysqld=...       mysqld binary to be used. Note that you can give mysqld_safe
                   ...
                   give this option inside groups [mysqld#] in ~/.mariadb.conf,
