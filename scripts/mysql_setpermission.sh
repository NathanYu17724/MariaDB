...
read_mariadb.conf();		# Read options from ~/.mariadb.conf
...
###
# Read variables password, port and socket from .mariadb.conf under the client
# or perl groups
###

sub read_mariadb.conf
{
  open(TMP,$ENV{'HOME'} . "/.mariadb.conf") || return 1;
  while (<TMP>)
  {
    if (/^\[(client|perl)\]/i)
    {
...
The permission setter first reads your .mariadb.conf file in your Home
...
If you don't give a password and no password is set in your .mariadb.conf
