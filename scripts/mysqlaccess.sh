...
END {
	unlink $MYSQL_CONF if defined $MYSQL_CONF and not $DEBUG;
}
...
if (defined($Param{'superuser'})) {
   $MYSQL_CONF{'mysql'}{'user'} = $Param{'superuser'};
   $MYSQL_CONF{'mysqldump'}{'user'} = $Param{'superuser'};
}
if (defined($Param{'spassword'})) {
   ...
   if ( length($Param{'spassword'}) > 0 ) {
      $MYSQL_CONF{'mysql'}{'password'} = $Param{'spassword'};
      $MYSQL_CONF{'mysqldump'}{'password'} = $Param{'spassword'};
   }
}
WriteTempConfigFile();
             ...
             if ( /\[\s*(\w+)\s*]/ ) {
                 ...
                 if ( defined $item ) {
                     ...
                     if ( $group eq 'client' || $group eq "client-server") {
                         $MYSQL_CONF{'mysql'}{$item} = $value;
                         $MYSQL_CONF{'mysqldump'}{$item} = $value;
                     } else {
                         $MYSQL_CONF{$group}{$item} = $value;
                     }
                 }
             }
         }
...
sub MergeConfigFiles {
    ...
    MergeConfigFile("@prefix@/mariadb.conf");
    MergeConfigFile("@sysconfdir@/mariadb.conf");
    MergeConfigFile("$dir/.mariadb.conf");
}
...
# =================================
sub WriteTempConfigFile {
   sysopen CONFFILE, $MYSQL_CONF, O_RDWR|O_CREAT|O_EXCL, 0700
      or die "sysopen $MYSQL_CONF: $!";
   
   # groups may be in any order, generic groups such as [client] assumed
   # here to be empty
   foreach $group (keys %MYSQL_CONF) {
      print CONFFILE "[$group]\n";
      foreach $item (keys %{$MYSQL_CONF{$group}}) {
         if ( defined $MYSQL_CONF{$group}{$item} ) {
            print CONFFILE "$item=$MYSQL_CONF{$group}{$item}\n";
         } else {
            print CONFFILE "$item\n";
         }
      }
...
sub OpenConnection {
    ...
    my $connect = "$MySQLaccess::MYSQL --defaults-file=$MySQLaccess::MYSQL_CONF";
...
#    print STDERR "CreateTable($table)\n";
    ...
    $connect .= " --defaults-file=$MySQLaccess::MYSQL_CONF --no-data";
    # superuser, spassword transmitted via defaults-file
    if (defined($MySQLaccess::Param{'rhost'}))     { $connect .= " --host=$MySQLaccess::Param{'rhost'}"; }
    $connect .= " $MySQLaccess::ACCESS_DB";
