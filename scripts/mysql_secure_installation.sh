...
# - single-quoted option values on the right hand side of = in mariadb.conf
#
# These two contexts don't handle escapes identically.  SQL strings allow
# quoting any character (\C => C, for any C), but mariadb.conf parsing allows
# quoting only \, ' or ".  For example, password='a\b' quotes a 3-character
# string in mariadb.conf, but a 2-character string in SQL.
#
...
#
# create a simple mariadb.conf file to be able to pass the root password to the mysql
