		       ...
		       uint port, const char *unix_socket,ulong client_flag)
{
  ...
  if (mysql->options.mariadb_conf_file || mysql->options.mariadb_conf_group)
  {
    mysql_read_default_options(&mysql->options,
				...
				mysql->options.mariadb_conf_file : "mariadb"),
			       mysql->options.mariadb_conf_group);
    my_free(mysql->options.mariadb_conf_file);
    my_free(mysql->options.mariadb_conf_group);
...
my_bool mysql_reconnect(MYSQL *mysql)
{
  ...
  tmp_mysql.options.mariadb_conf_file= tmp_mysql.options.mariadb_conf_group= 0;
...
mysql_options(MYSQL *mysql,enum mysql_option option, const void *arg)
{
  ...
  case MYSQL_READ_DEFAULT_FILE:
    my_free(mysql->options.mariadb_conf_file);
    mysql->options.mariadb_conf_file=my_strdup(arg,MYF(MY_WME));
    break;
  case MYSQL_READ_DEFAULT_GROUP:
    my_free(mysql->options.mariadb_conf_group);
    mysql->options.mariadb_conf_group=my_strdup(arg,MYF(MY_WME));
    break;
