...
static const char *ini_exts[]= {"ini", ".conf", 0};
#define R_OK 4
#else
#include <unistd.h>
static const char *ini_exts[]= {".conf", 0};
#endif
...
static char *get_installation_path() {
  char abs_path[PATH_MAX] = {0};
  char *abs_dir_path = NULL;
  int count = readlink("/proc/self/exe", abs_path, PATH_MAX);
  	
  if (count != -1) {
	abs_dir_path = dirname(abs_path);
	abs_dir_path = dirname(abs_dir_path);
  }
  
  return abs_dir_path;
}

static char *get_path_in_installation(const char *lookup_path) {
  char *path_ptr = NULL;
  ...
  char *install_dir = get_installation_path();
          
  if (install_dir!= NULL) {
    char *strored_dir_path = strdup(install_dir);
    install_dir = dirname(install_dir);
    char *sysroot = strdup(install_dir);
    const char *sysroot_usr = strcat(install_dir, "/usr");
    path_ptr = (char *)malloc(PATH_MAX + 1);
    
    if (path_ptr == NULL) {
      return path_ptr;
    }
    
    memset(path_ptr, 0, PATH_MAX + 1);

    if (strcmp(sysroot, "/") != 0 && strcmp(strored_dir_path, sysroot_usr) == 0) {
      memcpy(path_ptr, sysroot, strlen(sysroot));
      memcpy(path_ptr + strlen(sysroot), "/", strlen("/"));
      memcpy(path_ptr + strlen(sysroot) + strlen("/"), lookup_path, strlen(lookup_path));
    } else {
      memcpy(path_ptr, strored_dir_path, strlen(strored_dir_path));
      memcpy(path_ptr + strlen(strored_dir_path), "/", strlen("/"));
      memcpy(path_ptr + strlen(strored_dir_path) + strlen("/"), lookup_path, strlen(lookup_path));
    }
  }
...
char **get_default_configuration_dirs()
{
  ...
  */
  ...
  char *path_ptr = get_path_in_installation(DEFAULT_SYSCONFDIR);

  if (path_ptr != NULL) {
    if (add_cfg_dir(configuration_dirs, path_ptr))
      goto error;
  }
#else
  if (add_cfg_dir(configuration_dirs, "/etc"))
    goto error;
  if (add_cfg_dir(configuration_dirs, "/etc/mariadb"))
    goto error;

  char *path_ptr = get_path_in_installation("/etc");
  ...
  path_ptr = get_path_in_installation("/etc/mariadb");
...
#endif
#endif
/* This differs from https://mariadb.com/kb/en/mariadb/configuring-mariadb-with-m.conf/ where MARIADB_HOME is not specified for Windows */
    ...
    if (*ptr == '!')
    {
      ...
      *end= 0;
      if (!strcmp(ptr, "includedir")) {
        int errnum = _mariadb_read_options(mysql, (const char *)val, NULL, group, recursion + 1);
        
        /* If not find directory, try search directory in installation directory. */
        if (errnum == 1) {
          char *path_ptr = get_path_in_installation((const char *)val);
          
          if (path_ptr != NULL) {
            _mariadb_read_options(mysql, (const char *)path_ptr, NULL, group, recursion + 1);
          }
        }
      } else if (!strcmp(ptr, "include")) {
        int errnum = _mariadb_read_options(mysql, NULL, (const char *)val, group, recursion + 1);
        
        /* If not find file, try search file in installation directory. */
        if (errnum == 1) {
          char *path_ptr = get_path_in_installation((const char *)val);
          
          if (path_ptr != NULL) {
            _mariadb_read_options(mysql, NULL, (const char *)path_ptr, group, recursion + 1);
          }
        }
      }
      continue;
                              ...
                              unsigned int recursion)
{
  ...
  if (config_dir && config_dir[0])
  {
    for (exts= 0; ini_exts[exts]; exts++)
    {
      snprintf(filename, FN_REFLEN,
               "%s%cmariadb.%s", config_dir, FN_LIBCHAR, ini_exts[exts]);
  ...
  for (i=0; i < MAX_CONFIG_DIRS && configuration_dirs[i]; i++)
  {
    ...
    {
      snprintf(filename, FN_REFLEN,
               "%s%cmariadb.%s", configuration_dirs[i], FN_LIBCHAR, ini_exts[exts]);
      if (!access(filename, R_OK))
        errors+= _mariadb_read_options_from_file(mysql, filename, group, recursion);
    }
  }
#ifndef _WIN32
  /* special case: .mariadb.conf in Home directory */
  if ((env= getenv("HOME")))
  {
    for (exts= 0; ini_exts[exts]; exts++)
    {
      snprintf(filename, FN_REFLEN,
               "%s%c.mariadb.%s", env, FN_LIBCHAR, ini_exts[exts]);
      if (!access(filename, R_OK))
        errors+= _mariadb_read_options_from_file(mysql, filename, group, recursion);
    }
  }
