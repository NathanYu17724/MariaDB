		   ...
		   uint port, const char *unix_socket, unsigned long client_flag)
{
  ...
  /* use default options */
  if (mysql->options.mariadb_conf_file || mysql->options.mariadb_conf_group)
  {
    _mariadb_read_options(mysql, NULL,
			  (mysql->options.mariadb_conf_file ?
			   mysql->options.mariadb_conf_file : NULL),
			   mysql->options.mariadb_conf_group, 0);
    free(mysql->options.mariadb_conf_file);
    free(mysql->options.mariadb_conf_group);
    mysql->options.mariadb_conf_file=mysql->options.mariadb_conf_group=0;
  }
...
my_bool STDCALL mariadb_reconnect(MYSQL *mysql)
{
  ...
  tmp_mysql.options.mariadb_conf_group= tmp_mysql.options.mariadb_conf_file= NULL;
...
mysql_optionsv(MYSQL *mysql,enum mysql_option option, ...)
{
  ...
  case MYSQL_READ_DEFAULT_FILE:
    OPT_SET_VALUE_STR(&mysql->options, mariadb_conf_file, (char *)arg1);
    break;
  case MYSQL_READ_DEFAULT_GROUP:
    OPT_SET_VALUE_STR(&mysql->options, mariadb_conf_group, arg1 ? (char *)arg1 : "");
    break;
...
mysql_get_optionv(MYSQL *mysql, enum mysql_option option, void *arg, ...)
{
  ...
  case MYSQL_INIT_COMMAND:
    ...
    break;
  case MYSQL_READ_DEFAULT_FILE:
    *((char **)arg)= mysql->options.mariadb_conf_file;
    break;
  case MYSQL_READ_DEFAULT_GROUP:
    *((char **)arg)= mysql->options.mariadb_conf_group;
    break;
  case MYSQL_SET_CHARSET_DIR:
