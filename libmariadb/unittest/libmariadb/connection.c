...
static int test_conc66(MYSQL *my)
{
  ...
  if (!(fp= fopen("./my-conc66-test.conf", "w")))
    return FAIL;
  ...
  rc= mysql_options(mysql, MYSQL_READ_DEFAULT_FILE, "./my-conc66-test.conf");
  check_mysql_rc(rc, mysql);
...
static int test_mdev13100(MYSQL *my __attribute__((unused)))
{
  ...
  rc= mysql_options(mysql, MYSQL_READ_DEFAULT_FILE, "./mdev13100.conf");
  check_mysql_rc(rc, mysql);
...
static int test_conc317(MYSQL *unused __attribute__((unused)))
{
  ...
  snprintf(conf_file1, FN_REFLEN, "%s%c.mariadb.conf", env, FN_LIBCHAR);

  FAIL_IF(!access(conf_file1, R_OK), "access");

  mysql= mysql_init(NULL);
  fp= fopen(conf_file1, "w");
  FAIL_IF(!fp, "fopen");
...
static int test_conc327(MYSQL *unused __attribute__((unused)))
{
  ...
  snprintf(conf_file1, FN_REFLEN, "%s%c.mariadb.conf", env, FN_LIBCHAR);
  snprintf(conf_file2, FN_REFLEN, "%s%c.mariadb.tmp", env, FN_LIBCHAR);

  FAIL_IF(!access(conf_file1, R_OK), "access");

  fp1= fopen(conf_file1, "w");
  fp2= fopen(conf_file2, "w");
  ...
  snprintf(conf_file1, FN_REFLEN, "%s%cmariadb.conf", env, FN_LIBCHAR);
  fp1= fopen(conf_file1, "w");
  fp2= fopen(conf_file2, "w");
  FAIL_IF(!fp1 || !fp2, "fopen failed");
  ...
  fclose(fp2);
  mysql= mysql_init(NULL);
  mysql_options(mysql, MYSQL_READ_DEFAULT_FILE, conf_file2);
  my_test_connect(mysql, hostname, username, password,
                  schema, 0, socketname, 0);
