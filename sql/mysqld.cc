...
char mariadb_home[FN_REFLEN], pidfile_name[FN_REFLEN], system_time_zone[30];
...
char *mariadb_home_ptr, *pidfile_name_ptr;
...
#else
    mysqld_unix_port= (char*) MARIADB_UNIX_ADDR;
#endif
                  ...
                  const char *ext)
{
  ...
  /* mysql_real_data_home_ptr  may be null if no value of datadir has been
     specified through command-line or througha.conf file. If that is the
  ...
  */
  ...
  if (WSREP_ON && !wsrep_recovery && !opt_abort)
  {
    ...
    else // full wsrep initialization
    {
      ...
      char* const tmp_path= (char*)my_alloca(strlen(mariadb_home) +
                                             strlen("/bin") + 1);
      if (tmp_path)
      {
        strcpy(tmp_path, mariadb_home);
        strcat(tmp_path, "/bin");
        wsrep_prepend_PATH(tmp_path);
      }
      else
      {
        WSREP_ERROR("Could not append %s/bin to PATH", mariadb_home);
      }
      my_afree(tmp_path);
...
#endif
{
  ...
  */
  ...
  load_defaults_or_exit(MARIADB_CONFIG_NAME, load_default_groups, &argc, &argv);
...
struct my_option my_long_options[]=
{
  ...
  */
  ...
  {"skip-bdb", OPT_DEPRECATED_OPTION,
   "Deprecated option; Exist only for compatibility with old mariadb.conf files",
   0, 0, 0, GET_NO_ARG, NO_ARG, 0, 0, 0, 0, 0, 0},
#ifndef DISABLE_GRANT_OPTIONS
  {"skip-grant-tables", 0,
...
static void usage(void)
{
  ...
  puts("");
#endif
  print_defaults(MARIADB_CONFIG_NAME,load_default_groups);
  puts("");
  set_ports();
...
static int mysql_init_variables(void)
{
  ...
  mariadb_home[0]= pidfile_name[0]= log_error_file[0]= 0;
  ...
  mysql_real_data_home_len=
    (uint)(strmake_buf(mysql_real_data_home,
                get_relative_path(MARIADB_DATADIR)) - mysql_real_data_home);
  ...
  /* Allow Win32 users to move MySQL anywhere */
  {
    ...
    cleanup_dirname(mariadb_home,prg_dev);
  }
#else
  const char *tmpenv;
  if (!(tmpenv = getenv("MARIADB_BASEDIR_VERSION")))
    tmpenv = DEFAULT_MARIADB_HOME;
  strmake_buf(mariadb_home, tmpenv);
  set_sys_var_value_origin(&mariadb_home_ptr, sys_var::ENV);
#endif
...
mysqld_get_one_option(int optid, const struct my_option *opt, char *argument)
{
  ...
  case OPT_DEPRECATED_OPTION:
    sql_print_warning("'%s' is deprecated. It does nothing and exists only "
                      "for compatibility with old mariadb.conf files.",
                      opt->name);
    break;
  ...
  case 'a':
    ...
    break;
  case 'b':
    strmake_buf(mariadb_home, argument);
    break;
  case 'C':
...
static char *get_relative_path(const char *path)
{
  if (test_if_hard_path(path) &&
      is_prefix(path,DEFAULT_MARIADB_HOME) &&
      strcmp(DEFAULT_MARIADB_HOME,FN_ROOTDIR))
  {
    path+=(uint) strlen(DEFAULT_MARIADB_HOME);
...
static int fix_paths(void)
{
  ...
  convert_dirname(mariadb_home,mariadb_home,NullS);
  /* Resolve symlinks to allow 'mariadb_home' to be a relative symlink */
  my_realpath(mariadb_home,mariadb_home,MYF(0));
  /* Ensure that mariadb_home ends in FN_LIBCHAR */
  pos=strend(mariadb_home);
  if (pos[-1] != FN_LIBCHAR)
  {
    pos[0]= FN_LIBCHAR;
    pos[1]= 0;
  ...
  (void) my_load_path(mariadb_home,mariadb_home,""); // Resolve current dir
  (void) my_load_path(mysql_real_data_home,mysql_real_data_home,mariadb_home);
  (void) my_load_path(pidfile_name, pidfile_name_ptr, mysql_real_data_home);

  convert_dirname(opt_plugin_dir, opt_plugin_dir_ptr ? opt_plugin_dir_ptr : 
                                  get_relative_path(PLUGINDIR), NullS);
  (void) my_load_path(opt_plugin_dir, opt_plugin_dir, mariadb_home);
  ...
  if (test_if_hard_path(sharedir))
    ...
    strxnmov(buff,sizeof(buff)-1,mariadb_home,sharedir,NullS);
