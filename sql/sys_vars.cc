...
static Sys_var_charptr Sys_basedir(
       ...
       READ_ONLY GLOBAL_VAR(mariadb_home_ptr), CMD_LINE(REQUIRED_ARG, 'b'),
...
static bool check_log_path(sys_var *self, THD *thd, set_var *var)
{
  ...
  static const LEX_CSTRING mariadb_conf= { STRING_WITH_LEN("mariadb.conf") };
  static const LEX_CSTRING my_ini= { STRING_WITH_LEN("mariadb.ini") };
  if (path_length >= mariadb_conf.length)
  {
    if (strcasecmp(path + path_length - mariadb_conf.length, mariadb_conf.str) == 0)
      return true; // log file name ends with "mariadb.conf"
    DBUG_ASSERT(mariadb_conf.length == my_ini.length);
    if (strcasecmp(path + path_length - my_ini.length, my_ini.str) == 0)
      return true; // log file name ends with "mariadb.ini"
  }
