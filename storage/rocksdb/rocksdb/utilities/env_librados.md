...
mkdir -p /etc/mariadb/conf.d
echo -e '[mysqld_safe]\nsyslog' > /etc/mariadb/conf.d/mysqld_safe_syslog.conf
cp /usr/share/mysql/my-medium.conf /etc/mariadb/mariadb.conf
sed -i 's#.*datadir.*#datadir = /var/lib/mysql#g' /etc/mariadb/mariadb.conf
