...
sub combinations_from_file($$)
{
  ...
  if ($skip_combinations{$filename}) {
    ...
    # Read combinations file in mariadb.conf format
...
sub collect_one_test_case {
  ...
  # ----------------------------------------------------------------------
  my $test.conf_file= find_file_in_dirs($tinfo, tdir => "$tname.conf");
  if ($test.conf_file ) {
    # Specifies the configuration file to use for this test
    $tinfo->{'template_path'}= $test.conf_file;
  }
  ...
  {
    my $config= find_file_in_dirs($tinfo, dir => 'mariadb.conf');
    if (not $config)
    {
      # Suite has no config, autodetect which one to use
      if ($tinfo->{rpl_test}) {
        $config= "suite/rpl/mariadb.conf";
      } else {
        $config= "include/default_mariadb.conf";
      }
    }
