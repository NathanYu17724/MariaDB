...
my $test.conf= "$dir/test.conf";

# Write test config file
open(OUT, ">", $test.conf) or die;
print $test.conf, "\n";
...
my $config= My::Config->new($test.conf);
...
my $test2.conf= "$dir/test2.conf";
$config->save($test2.conf);

# read it back and check it's the same
my $config2= My::Config->new($test2.conf);
isa_ok( $config2, "My::Config" );
is_deeply( \$config, \$config2, "test.conf is equal to test2.conf");


my $test_include.conf= "$dir/test_include.conf";
# Write test config file that includes test.conf
open(OUT, ">", $test_include.conf) or die;
...
my $config3= My::Config->new($test_include.conf);
isa_ok( $config3, "My::Config" );
print $config3;
is( $config3->value('mysqld', 'basedir'), 'anotherbasedir',
    "mysqld_basedir has been overridden by value in test_include.conf");

is( $config3->value('mysqld', 'option1'), 'values3',
    "mysqld_option1 has been overridden by value in test_include.conf");
