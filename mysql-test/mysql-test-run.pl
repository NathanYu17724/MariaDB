...
my $path_config_file;           # The generated config file, var/mariadb.conf
...
sub set_vardir {
  ...
  # Location of mariadb.conf that all clients use
  $path_config_file= "$opt_vardir/mariadb.conf";
...
sub create_config_file_for_extern {
  ...
  mtr_report("Creating mariadb.conf file for extern server...");
...
sub default_mysqld {
  ...
  my $config= My::ConfigFactory->new_config
    ( {
       ...
       template_path   => "include/default_mariadb.conf",
       vardir          => $opt_vardir,
       tmpdir          => $opt_tmpdir,
       baseport        => 0,
       user            => $opt_user,
...
sub run_testcase ($$) {
  ...
  if ( ! using_extern() )
  {
	...
	if (defined $value){
      ...
      mtr_verbose("Generating mariadb.conf from '$tinfo->{template_path}'");
