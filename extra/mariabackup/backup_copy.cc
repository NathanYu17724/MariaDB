...
bool backup_start()
{
	if (!opt_no_lock) {
		...
		if (!backup_files(fil_path_to_mariadb_datadir, true)) {
			return(false);
		}
	...
	if (!backup_files(fil_path_to_mariadb_datadir, false)) {
		return(false);
	}

	if (!backup_files_from_datadir(fil_path_to_mariadb_datadir)) {
		return false;
	}
...
copy_back()
{
	...
	while (datadir_iter_next(it, &node)) {
		const char *ext_list[] = {"backup-mariadb.conf",
