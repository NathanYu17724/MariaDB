...
/*
  ...
  Elsewhere, the compiled-in constant MARIADB_DATADIR is used.
*/
static char *get_default_datadir() {
	static char ddir[] = MARIADB_DATADIR;
#ifdef _WIN32
...
static int prepare_export()
{
  ...
  if (strncmp(orig_argv1,"--defaults-file=",16) == 0)
  {
    snprintf(cmdline, sizeof cmdline,
     IF_WIN("\"","") "\"%s\" --mysqld \"%s\" "
      " --defaults-extra-file=./backup-mariadb.conf --defaults-group-suffix=%s --datadir=."
  ...
  else
  {
    sprintf(cmdline,
     IF_WIN("\"","") "\"%s\" --mysqld"
      " --defaults-file=./backup-mariadb.conf --defaults-group-suffix=%s --datadir=."
      " --innodb --innodb-fast-shutdown=0 --loose-partition"
      " --innodb_purge_rseg_truncate_frequency=1 --innodb-buffer-pool-size=%llu"
...
You can download full text of the license on http://www.gnu.org/licenses/gpl-2.0.txt\n");
  ...
  print_defaults("mariadb", xb_server_default_groups);
...
static bool innodb_init_param()
{
	...
	fil_path_to_mariadb_datadir = default_path;

	/* Set InnoDB initialization parameters according to the values
	read from MariaDB .conf file */
	...
	/* The default dir for data files is the datadir of MariaDB/MySQL */
...
static dberr_t enumerate_ibd_files(process_single_tablespace_func_t callback)
{
	...
	dir = os_file_opendir(fil_path_to_mariadb_datadir, true);
	...
	ret = fil_file_readdir_next_file(&err, fil_path_to_mariadb_datadir, dir,
					 &dbinfo);
	while (ret == 0) {
		...
		len = strlen(fil_path_to_mariadb_datadir)
		...
		snprintf(dbpath, dbpath_len,
			 "%s/%s", fil_path_to_mariadb_datadir, dbinfo.name);
...
handle_options(int argc, char **argv, char ***argv_client, char ***argv_server)
{
	...
	snprintf(conf_file, sizeof(conf_file), "mariadb");

	if (prepare && target_dir) {
		snprintf(conf_file, sizeof(conf_file),
			 "%s/backup-mariadb.conf", target_dir);
