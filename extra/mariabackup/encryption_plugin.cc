...
/*
  Read "plugin-load" value (encryption plugin) from backup-mariadb.conf during
  prepare phase.
  The value is stored during backup phase.
*/
static std::string get_encryption_plugin_from_conf()
{
  FILE *f = fopen("backup-mariadb.conf", "r");
  if (!f)
  {
    die("Can't open backup-mariadb.conf for reading");
  }
  char line[512];
...
{
  std::string plugin_load= get_encryption_plugin_from_conf();
  if (plugin_load.size())
  {
