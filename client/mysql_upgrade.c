...
static char *conf_file_path= 0, defaults_file[FN_REFLEN + 32];
...
#define PASSWORD_OPT 12
  ...
  {"port", 'P', "Port number to use for connection or 0 for default to, in "
   "order of preference, mariadb.conf, $MYSQL_TCP_PORT, "
...
static void free_used_memory(void)
{
  ...
  if (conf_file_path)
    my_delete(conf_file_path, MYF(MY_WME));
}
...
/*
  ...
  or by defaults file(mariadb.conf) - to a dynamic string, in
...
static void add_one_option_conf_file(DYNAMIC_STRING *ds,
                                    const struct my_option *opt,
                                    const char* arg)
{
  dynstr_append(ds, opt->name);
               ...
               char *argument)
{
  ...
  case '?':
    ...
    print_defaults("mariadb", load_default_groups);
    puts("");
  ...
  case 'p':
    ...
    if (argument)
    {
      ...
      add_one_option_conf_file(&ds_args, opt, argument);
      while (*argument)
...
int main(int argc, char **argv)
{
  ...
  MY_INIT(argv[0]);
  load_defaults_or_exit("mariadb", load_default_groups, &argc, &argv);
  ...
  if (tty_password)
  {
    ...
    add_one_option_conf_file(&ds_args, &my_long_options[PASSWORD_OPT], opt_password);
    DBUG_ASSERT(strcmp(my_long_options[PASSWORD_OPT].name, "password") == 0);
  }
  /* add user to defaults file */
  add_one_option_conf_file(&ds_args, &my_long_options[USER_OPT], opt_user);
  DBUG_ASSERT(strcmp(my_long_options[USER_OPT].name, "user") == 0);

 conf_file_path= strmov(defaults_file, "--defaults-file=");
  {
    int fd= create_temp_file(conf_file_path, opt_tmpdir[0] ? opt_tmpdir : NULL,
