...
-	On Unix, edit my the mariadb.conf/my.ini configuration file, set the parameter gssapi-keytab-path to point to previously
created keytab path.
...
1. Server : Construct gssapi-principal-name if not set in mariadb.conf. On Unixes defaults to hostbased name for service "mariadb". On Windows to user's or machine's domain names.
