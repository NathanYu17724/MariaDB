...
# - Create a /etc/mariadb.conf file with the following information:
#   [mysqld]
...
# in the /etc/mariadb.conf, ~/.mariadb.conf or other MariaDB configuration files.
...
# for server start.
# Value here is overridden by value in mariadb.conf.
...
# Get arguments from the mariadb.conf file,
# the only group, which is read from now on is [mysqld]
if test -x "$bindir/my_print_defaults";  then
  print_defaults="$bindir/my_print_defaults"
else
  # Try to find basedir in /etc/mariadb.conf
  conf=/etc/mariadb.conf
  print_defaults=
  if test -r $conf
  then
    subpat='^[^=]*basedir[^=]*=\(.*\)$'
...
extra_args=""
if test -r "$basedir/mariadb.conf"
then
  extra_args="-e $basedir/mariadb.conf"
else
  if test -r "$datadir/mariadb.conf"
  then
    extra_args="-e $datadir/mariadb.conf"
  fi
fi
...
case "$mode" in
  'start')
    ...
    then
      # Give extra arguments to mysqld with the mariadb.conf file. This script
