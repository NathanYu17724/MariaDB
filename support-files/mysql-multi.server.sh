...
# prefix configuration variables in a separate section of /etc/mariadb.conf.
# See example below.
#
# A typical multi-instance /etc/mariadb.conf file would look like:
# [mysqld]
# basedir=...
# key_buffer_size=16M
...
#
# and then the /foo1/bar/mariadb.conf and /foo2/bar/mariadb.conf files
...
# Get arguments from the mariadb.conf file, groups [mysqld], [mysql_server],
...
case "$mode" in
  'start')
    ...
    then
      ...
      # get all other instance-specific config from $datadir/mariadb.conf.
      ...
      $bindir/mysqld_safe --defaults-extra-file=$datadir/mariadb.conf \
